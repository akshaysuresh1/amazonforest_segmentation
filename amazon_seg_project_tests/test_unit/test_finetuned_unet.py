...
@patch("amazon_seg_project.assets.model.write_loss_data_to_csv")
@patch("logging.info")
def test_finetuned_unet_early_stopping_triggered(
    mock_logging: MagicMock,
    mock_write_loss_data: MagicMock,
    mock_save_model_weights: MagicMock,
    mock_validate_epoch: MagicMock,
    ...
    """
    Test early stopping of model finetuning
    """
    # Set up config, datasets, and pretrained model.
    config = FinetunedUnetConfig(max_epochs=8, batch_size=4, lr_initial=0.001)
    ...
    mock_save_model_weights.assert_called_with(model, OUTPUT_PATH / "model_weights.pt")
    mock_write_loss_data.assert_called_once_with(
        mock_train_loss[:-1],
        mock_val_loss[:-1],
        OUTPUT_PATH / "train" / "loss_curve.csv",
    ...
    # Logging assertions for epochs 1 – 2
    expected_calls = [
        call(
            "Starting training:\n"
            "Max no. of epochs = %d\n"
    ...
    # Logging for epochs 3 – 7
    for epoch in range(3, config.max_epochs):
        expected_calls.append(
            call(
                "Statistics for epoch %d: \n Training loss: %.4f \n Validation loss: %.4f",
                epoch,
                mock_train_loss[epoch - 1],
                mock_val_loss[epoch - 1],
            )
        )
        expected_calls.append(
            call(
