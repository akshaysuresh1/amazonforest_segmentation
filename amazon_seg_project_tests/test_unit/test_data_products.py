...
@patch("amazon_seg_project.assets.data_products.load_labeled_data")
def test_training_data_absent(mock_load_labeled_data: MagicMock) -> None:
    """
...
def test_validation_data_success(mock_load_labeled_data: MagicMock) -> None:
    ...
    # First output element: val_image_files
    ...
    # Second output element: val_mask_files
...
@patch("amazon_seg_project.assets.data_products.load_labeled_data")
def test_validation_data_absent(mock_load_labeled_data: MagicMock) -> None:
    """
...
@patch("amazon_seg_project.assets.data_products.load_labeled_data")
def test_testdata_success(mock_load_labeled_data: MagicMock) -> None:
    """
    ...
    # First output element: test_image_files
    assert outputs[0].output_name == "test_image_files", (
        "The name of the first output is not 'test_image_files'."
    )
    assert outputs[0].value == mock_test_image_files, (
        f"Expected output: {mock_test_image_files}, Actual output: {outputs[0].value}'"
    )
    assert outputs[0].metadata.get("Count of images").value == len(
        mock_test_image_files
    ...
    # Second output element: test_mask_files
    assert outputs[1].output_name == "test_mask_files", (
        "The name of the second output is not 'test_mask_files'."
    )
    assert outputs[1].value == mock_test_mask_files, (
        f"Expected output: {mock_test_mask_files}, Actual output: {outputs[1].value}'"
    )
    assert outputs[1].metadata.get("Count of masks").value == len(
        mock_test_mask_files
    ), f"""Expected image count = {len(mock_test_mask_files)},
        Actual image count = {len(outputs[1].value)}"""


@patch("amazon_seg_project.assets.data_products.load_labeled_data")
def test_testdata_absent(mock_load_labeled_data: MagicMock) -> None:
    """
    Evaluate response of data_test() to an empty list of data files.
    """
    with pytest.raises(
        ValueError,
        match=f"Zero .tif images with masks found at {str(TEST_IMAGES_PREFIX)}",
    ):
        # Set up empty list of data files and mask files.
        mock_load_labeled_data.return_value = ([], [])
